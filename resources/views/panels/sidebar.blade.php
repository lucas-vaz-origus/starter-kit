<aside
    ...
    <h1 class="logo-wrapper">
      <a class="brand-logo darken-1" href="{{asset('/')}}">
        @if(!empty($configData['mainLayoutType']) && isset($configData['mainLayoutType']))
          ...
          <img id="idFoto" class="hide-on-med-and-down" src="{{asset($configData['largeScreenLogo'])}}" alt="materialize logo" />
    ...
    @if(!empty($menuData[0]) && isset($menuData[0]))
      @foreach ($menuData[0]->menu as $menu)
        ...
        <li class="navigation-header">
          <a class="navigation-header-text waves-effect waves-light btn gradient-45deg-lime-deep-green-dark center sidenav-trigger" href="#" data-target="slide-out-right">{{ $menu->navheader }}</a>
  ...
  <a class="sidenav-trigger btn-sidenav-toggle btn-floating light-green darken-1 btn-medium waves-effect waves-light hide-on-large-only botao-menu-lateral"
    href="#" data-target="slide-out"><i class="icon-botao-menu-lateral material-icons">menu</i></a>
</aside>
